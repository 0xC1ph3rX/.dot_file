return {
	{
		...
		config = function()
			...
			-- Prefer local clangd from goinfre, otherwise fall back to mason/system clangd
			local clangd_cmd = { "clangd", "--background-index", "--clang-tidy" }
			local goinfre_clangd = "/goinfre/mtarza/clang+llvm-18.1.8-x86_64-linux-gnu-ubuntu-18.04/bin/clangd"
			local mason_clangd = vim.fn.stdpath("data") .. "/mason/bin/clangd"
			if vim.fn.executable(goinfre_clangd) == 1 then
				clangd_cmd[1] = goinfre_clangd
			elseif vim.fn.executable(mason_clangd) == 1 then
